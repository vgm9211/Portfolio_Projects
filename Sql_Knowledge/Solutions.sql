...
SELECT 
    to_date,
    from_date,
    to_date - from_date AS just_a_subtraction,
    TIMESTAMPDIFF(SECOND,from_date,to_date) AS diff_in_seconds,
    DATEDIFF(to_date, from_date) AS diff_in_days,
    60 * 60 * 24 * 7 * 52 + 60 * 60 * 24 AS seconds_in_year,
    7 * 52 + 1 AS days_in_year
FROM
    salaries;
